...
const TeacherDetail = () => {
  ...
  const [scheduleFilter, setScheduleFilter] = useState('all'); // Lọc theo loại lịch (hôm nay, sắp tới, đã qua)
  ...
  const fetchTeacherInfo = async () => {
    ...
    try {
      ...
      message.error("Lỗi khi tải thông tin giáo viên.");
    }
    setLoading(false);
  };
  ...
  const fetchAssignedClasses = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/teachers/${id}/classes`, {
        headers: getAuthHeaders(),
      });
  ...
  // Lấy lịch dạy của giáo viên (hôm nay, sắp tới, đã qua)
  const fetchTeacherSchedules = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/teachers/${id}/schedules`, {
        headers: getAuthHeaders(),
      });
      setSchedules(response.data);
    } catch (error) {
      message.error("Lỗi khi tải lịch dạy.");
    }
  };
  ...
  // Dùng useEffect để gọi các API khi component được mount
  useEffect(() => {
    fetchTeacherInfo();
  ...
  const handleOk = async () => {
    try {
      ...
      // Kiểm tra ngày sinh, nếu không có thay đổi, giữ lại ngày sinh cũ
      const payload = {
        ...values,
        date_of_birth: values.dob ? values.dob.format("YYYY-MM-DD") : teacherInfo.date_of_birth,
      };

      await axios.put(`${API_BASE_URL}/teachers/${id}`, payload, {
        headers: getAuthHeaders(),
      });
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {/* Tabs: Các lớp giảng dạy và lịch dạy */}
